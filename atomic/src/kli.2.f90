...
contains
            ...
            do i = 1, num_wf(s)
                ...
                do j = 1, num_wf(s)
                    ...
                    if( s_i == s_j) then
                        ...
                        do k = 1, grid_size(s)
                            func(k) = psi(k,1,idx_i)**2 * psi(k,1,idx_j)**2
                        ...
                        fact1 = 1 !(2 * orbital_angular_momentum(idx_i)  + 1)
                        fact2 =  1 !shell_occupancy(idx_j)
                        ...
                        ! retval goes to the i,j entry of matrix with spin s
                         mat_m(i,j,s) = retval  !- oc(idx_i)*oc(idx_j)/oc2(s) 
    ...
    subroutine  compute_potential_s(grid_size)
        ...
        do j = 1, num_wave_functions
            ...
            ! work = 0.0
            work =  psi(1:grid_size(s), 1, j) * slater_potential(1:grid_size(s),s)
            ! work = work * ( shell_occupancy(j) * psi(:, 1, j )) / rho(:,1)
            potential_s(j) = int_0_inf_dr(work, grid, grid_size(s), nst)  !* shell_occupancy(j)
#ifdef DEBUG
    ...
    subroutine compute_average_ux_kli(grid_size)
        ...
        do i  = 1, num_wave_functions
            ...
            s = get_spin(i)
            average_ux_kli(i) =  int_0_inf_dr(work, grid, grid_size(s) ,  nst) 
    ...
    integer :: last_r(2)
        ...
        do j = 1, grid_size
            ...
            enddo
            if( rho(j,1) > tiny(1.0_dp)) last_r(1) = j
            if( rho(j,2) > tiny(1.0_dp)) last_r(2) = j 
        enddo
        do s = 1,nspin
            if( last_r(s) == 0 .and. num_wf(s) /= 0) then
                print *, s, last_r(s), grid_size
                print *, rho(1:grid_size,s)
                stop "invalid "
            endif
        enddo
        ...
        ! M(:,:,1)  Matrix up
        ! M(:,:,2)  Matrix down
        call compute_mat_m(num_wave_functions, last_r,rho)    

        call compute_ux_kli(last_r)
        ! compute the average slater potential
        call compute_potential_s(last_r)
        call compute_average_ux_kli(last_r)
        ...
        do s = 1,nspin
            exchange_potential(last_r(s) +1: grid_size,s)  = - 1/grid%r(last_r(s) +1: grid_size)
            work = 0
            do i = 1, num_wf(s)  - 1 
                fact = ysol(idx(i,s))  !average_kli_potential(idx(i,s)) - average_ux_kli(idx(i,s))
                ! work =  work + abs(psi(:,1,i))**2 * fact * shell_occupancy(i)
                work(1:last_r(s),s) =  work(1:last_r(s),s) + psi(1:last_r(s),1,idx(i,s))* psi(1:last_r(s),1,idx(i,s)) * fact !shell_occupancy(idx(i,s))
            ...
            if(num_wf(s) > 0) then ! s can be zero as in the hidrogen case
                i = idx(num_wf(s),s)
                ! work(1:last_r(s),s) =  (work(1:last_r(s),s) )/rho(1:last_r(s),s)  - ysol(s)/ oc(i)
